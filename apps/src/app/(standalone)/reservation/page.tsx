...
const ServiceCard: React.FC<ServiceCardProps> = ({ title, description, isNew, className }) => (
  <Link href="/reservation/form" className={`block bg-[#F8F8F8] rounded-xl p-4 ${className}`}>
    ...
    {description.map((text, index) => (
      <p key={index} className="text-sm text-[#666666] mt-2">{text}</p>
    ))}
  </Link>
);
...
const ServiceIcon: React.FC<ServiceIconProps> = ({ name, icon, isNew, discount }) => (
